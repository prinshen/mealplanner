(() => {
  ...
  function openFoodModal(category, existing = null) {
    ...
    function renderFoodEditor() {
      ...
      const manualButton = document.getElementById('manual-food'); if (manualButton) manualButton.onclick = () => { draft = { ...draft, description: document.getElementById('food-text').value, manualWeightG: 0, per100Totals: { protein: 0, calories: 0 }, manualTotals: { protein: true, calories: true } }; entryMode = 'manual'; renderFoodEditor(); };
      ...
      const weightInput = document.getElementById('meal-weight'); if (weightInput) weightInput.oninput = e => { draft = applyManualWeight(draft, e.target.value); updateCalculatedTotal(); };
      if (existing) {
      ...
      document.getElementById('save-entry').onclick = () => {
        ...
        if (document.getElementById('save-to-saved').checked) {
          ...
          const savedMeal = { id: currentSaved?.id || uid(), name: entry.name, description, category, protein: entry.protein, calories: entry.calories, manualTotals: { ...entry.manualTotals }, ...(entry.manualWeightG != null ? { manualWeightG: entry.manualWeightG } : {}), ...(entry.per100Totals ? { per100Totals: { ...entry.per100Totals } } : {}), ingredients: clone(entry.ingredients), usageCount: currentSaved?.usageCount || 0 };
      ...
      function syncDraft() { draft.description = document.getElementById('food-text').value; if (root.querySelectorAll('.ingredient-row').length) draft.ingredients = readIngredientRows(); const weight = document.getElementById('meal-weight'); if (weight) draft = applyManualWeight(draft, weight.value); draft = applyEditorValue(draft, 'protein', document.getElementById('meal-protein').value); draft = applyEditorValue(draft, 'calories', document.getElementById('meal-calories').value); }
  ...
  function openSavedModal(id = null) {
    ...
    function renderBody() {
      ...
      const manualButton = document.getElementById('manual-saved'); if (manualButton) manualButton.onclick = () => { draft = { ...draft, name: document.getElementById('saved-name').value, description: document.getElementById('saved-name').value, category: document.getElementById('saved-category').value, manualWeightG: 0, per100Totals: { protein: 0, calories: 0 }, manualTotals: { protein: true, calories: true } }; entryMode = 'manual'; renderBody(); };
      const cancel = document.getElementById('cancel-saved'); if (cancel) cancel.onclick = closeModal;
      if (!showNutrition) return;
      const weightInput = document.getElementById('saved-weight'); if (weightInput) weightInput.oninput = e => { draft = applyManualWeight(draft, e.target.value); updateSavedTotal(); };
      ...
      function syncSavedDraft() { if (body.querySelectorAll('.ingredient-row').length) draft.ingredients = readIngredientRows(); const weight = document.getElementById('saved-weight'); if (weight) draft = applyManualWeight(draft, weight.value); draft = applyEditorValue(draft, 'protein', document.getElementById('saved-protein').value); draft = applyEditorValue(draft, 'calories', document.getElementById('saved-calories').value); }
