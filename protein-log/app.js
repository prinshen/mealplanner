(() => {
  ...
  function openFoodModal(category, existing = null) {
    ...
    function renderFoodResult() {
      ...
      root.innerHTML = `<div class="card result-summary"><strong>~${roundMacro(analyzed.protein)} g protein</strong><span>~${roundMacro(analyzed.carbs)} g carbs · ~${Math.round(analyzed.calories)} kcal</span></div>${analyzed.ingredients.length ? `<div class="section-kicker">Ingredients</div><div class="ingredient-head"><span>Ingredient</span><span>Amount</span><span>Protein /100g</span></div><div class="ingredient-list">${analyzed.ingredients.map((ing, i) => ingredientRowHtml(ing, i)).join('')}</div><div class="settings-help">Edit grams or protein per 100 g. The meal updates automatically.</div>` : ''}<div class="modal-actions">${existing ? '<button class="secondary-button danger" id="delete-entry">Delete</button>' : '<button class="secondary-button" id="cancel-entry">Cancel</button>'}<button class="primary-button" id="save-entry">${existing ? 'Save meal' : 'Add meal'}</button></div>`;
      root.querySelectorAll('.ingredient-row input').forEach(input => input.addEventListener('change', () => { analyzed.ingredients = readIngredientRows(); analyzed = calculateFromIngredients(analyzed); renderFoodResult(); }));
