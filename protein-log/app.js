(() => {
  ...
  function openFoodModal(category, existing = null) {
    ...
    let entryMode = existing ? (totalIngredientAmount(draft.ingredients) ? 'analyzed' : 'manual') : 'choice';
    ...
    function renderFoodEditor() {
      ...
      const nutrition = editorNutrition(draft);
      const showNutrition = entryMode !== 'choice';
      modalRoot._ingredients = draft.ingredients;
      const nutritionEditor = showNutrition ? `<div class="card total-editor"><div class="total-editor-head"><strong>${nutrition.usesPer100 ? 'Meal values per 100 g' : 'Manual nutrition'}</strong><small>${nutrition.usesPer100 ? 'Total calculated automatically' : 'Full meal values'}</small></div><div class="macro-grid"><div class="field"><label>${nutrition.usesPer100 ? 'Protein / 100 g' : 'Protein (g)'}</label><input id="meal-protein" inputmode="decimal" placeholder="0" value="${nutrition.usesPer100 || existing ? escapeAttr(roundInput(nutrition.proteinValue)) : ''}" /></div><div class="field"><label>${nutrition.usesPer100 ? 'Calories / 100 g' : 'Calories'}</label><input id="meal-calories" inputmode="decimal" placeholder="0" value="${nutrition.usesPer100 || existing ? escapeAttr(roundInput(nutrition.caloriesValue)) : ''}" /></div></div>${nutrition.usesPer100 ? `<div class="calculated-total" id="calculated-meal-total">For ${roundInput(nutrition.amount)} g: <strong>${roundMacro(draft.protein)} g protein · ${Math.round(draft.calories)} kcal</strong></div>` : ''}</div>` : '';
      const saveControls = showNutrition ? `${saveToSavedToggleHtml(saveToSavedMeals, Boolean(linkedSavedMeal))}${existing ? '<div class="copy-entry-action"><button class="secondary-button" id="copy-entry">Copy meal</button><small>Copy these values to paste on another day</small></div>' : ''}<div class="modal-actions">${existing ? '<button class="secondary-button danger" id="delete-entry">Delete</button>' : '<button class="secondary-button" id="cancel-entry">Cancel</button>'}<button class="primary-button" id="save-entry">${existing ? 'Save meal' : 'Add meal'}</button></div>${draft.ingredients.length ? `<details class="ingredient-details"><summary>Ingredient details <span>${draft.ingredients.length}</span></summary><div class="ingredient-head"><span>Ingredient</span><span>Amount</span></div><div class="ingredient-list">${draft.ingredients.map((ing, i) => ingredientRowHtml(ing, i)).join('')}</div><div class="settings-help">Adjust amounts or ingredient values here; the meal total is calculated for you.</div></details>` : ''}` : '<div class="modal-actions single"><button class="secondary-button" id="cancel-entry">Cancel</button></div>';
      root.innerHTML = `<div class="field"><label>Meal name or description</label><textarea id="food-text" placeholder="e.g. 100g oats with milk, 10 raisins and 5 almonds">${escapeHtml(draft.description || draft.name || '')}</textarea></div><div class="analysis-action"><button class="primary-button" id="analyze-food">Analyze with Claude</button>${entryMode === 'choice' ? '<button class="manual-entry-button" id="manual-food">Enter manually</button>' : '<small>Run again if the description changes</small>'}</div><div id="food-error"></div>${nutritionEditor}${saveControls}`;
      ...
      document.getElementById('analyze-food').onclick = async () => {
        ...
        try { const result = await analyzeFood(text, category); draft = { ...result, description: text, manualTotals: { protein: false, calories: false } }; analyzedWithAI = true; entryMode = 'analyzed'; renderFoodEditor(); }
      ...
      const manualButton = document.getElementById('manual-food'); if (manualButton) manualButton.onclick = () => { draft.description = document.getElementById('food-text').value; entryMode = 'manual'; renderFoodEditor(); };
      const cancel = document.getElementById('cancel-entry'); if (cancel) cancel.onclick = closeModal;
      if (!showNutrition) return;
      document.getElementById('meal-protein').oninput = e => { draft = applyEditorValue(draft, 'protein', e.target.value); updateCalculatedTotal(); };
      document.getElementById('meal-calories').oninput = e => { draft = applyEditorValue(draft, 'calories', e.target.value); updateCalculatedTotal(); };
      if (existing) {
      ...
      root.querySelectorAll('.ingredient-row input').forEach(input => input.addEventListener('input', () => {
        ...
        const updated = editorNutrition(draft);
        document.getElementById('meal-protein').value = roundInput(updated.proteinValue);
        document.getElementById('meal-calories').value = roundInput(updated.caloriesValue);
      ...
      document.getElementById('save-entry').onclick = () => {
        ...
        if (document.getElementById('save-to-saved').checked) {
          ...
          const savedMeal = { id: currentSaved?.id || uid(), name: entry.name, description, category, protein: entry.protein, calories: entry.calories, manualTotals: { ...entry.manualTotals }, ...(entry.per100Totals ? { per100Totals: { ...entry.per100Totals } } : {}), ingredients: clone(entry.ingredients), usageCount: currentSaved?.usageCount || 0 };
      ...
      function syncDraft() { draft.description = document.getElementById('food-text').value; if (root.querySelectorAll('.ingredient-row').length) draft.ingredients = readIngredientRows(); draft = applyEditorValue(draft, 'protein', document.getElementById('meal-protein').value); draft = applyEditorValue(draft, 'calories', document.getElementById('meal-calories').value); }
      function updateCalculatedTotal() { const el = document.getElementById('calculated-meal-total'), current = editorNutrition(draft); if (el && current.usesPer100) el.innerHTML = `For ${roundInput(current.amount)} g: <strong>${roundMacro(draft.protein)} g protein · ${Math.round(draft.calories)} kcal</strong>`; }
    }
  }
  ...
  function openSavedModal(id = null) {
    ...
    function renderBody() {
      ...
      const showNutrition = entryMode !== 'choice';
      modalRoot._ingredients = draft.ingredients;
      const nutritionEditor = showNutrition ? `<div class="card total-editor"><div class="total-editor-head"><strong>${nutrition.usesPer100 ? 'Meal values per 100 g' : 'Manual nutrition'}</strong><small>${nutrition.usesPer100 ? 'Total calculated automatically' : 'Full meal values'}</small></div><div class="macro-grid"><div class="field"><label>${nutrition.usesPer100 ? 'Protein / 100 g' : 'Protein (g)'}</label><input id="saved-protein" inputmode="decimal" placeholder="0" value="${nutrition.usesPer100 || existing ? escapeAttr(roundInput(nutrition.proteinValue)) : ''}" /></div><div class="field"><label>${nutrition.usesPer100 ? 'Calories / 100 g' : 'Calories'}</label><input id="saved-calories" inputmode="decimal" placeholder="0" value="${nutrition.usesPer100 || existing ? escapeAttr(roundInput(nutrition.caloriesValue)) : ''}" /></div></div>${nutrition.usesPer100 ? `<div class="calculated-total" id="calculated-saved-total">For ${roundInput(nutrition.amount)} g: <strong>${roundMacro(draft.protein)} g protein · ${Math.round(draft.calories)} kcal</strong></div>` : ''}</div>` : '';
      const saveControls = showNutrition ? `<div class="modal-actions">${existing ? '<button class="secondary-button danger" id="delete-saved">Delete</button>' : '<button class="secondary-button" id="cancel-saved">Cancel</button>'}<button class="primary-button" id="save-saved">Save</button></div>${draft.ingredients.length ? `<details class="ingredient-details"><summary>Ingredient details <span>${draft.ingredients.length}</span></summary><div class="ingredient-head"><span>Ingredient</span><span>Amount</span></div><div class="ingredient-list">${draft.ingredients.map((ing, i) => ingredientRowHtml(ing, i)).join('')}</div><div class="settings-help">Adjust amounts or ingredient values here; the meal total is calculated for you.</div></details>` : ''}` : '<div class="modal-actions single"><button class="secondary-button" id="cancel-saved">Cancel</button></div>';
      body.innerHTML = `<div class="field"><label>Name</label><input id="saved-name" placeholder="e.g. Huel Black, two scoops" value="${escapeAttr(draft.name || draft.description || '')}" /></div><div class="field"><label>Default meal</label><select id="saved-category">${mealOptions(draft.category || 'breakfast')}</select></div><div class="analysis-action"><button class="primary-button" id="analyze-saved">Analyze with Claude</button>${entryMode === 'choice' ? '<button class="manual-entry-button" id="manual-saved">Enter manually</button>' : '<small>Run again if the name changes</small>'}</div><div id="saved-error"></div>${nutritionEditor}${saveControls}`;
      ...
      const manualButton = document.getElementById('manual-saved'); if (manualButton) manualButton.onclick = () => { draft.name = document.getElementById('saved-name').value; draft.description = draft.name; draft.category = document.getElementById('saved-category').value; entryMode = 'manual'; renderBody(); };
      const cancel = document.getElementById('cancel-saved'); if (cancel) cancel.onclick = closeModal;
      if (!showNutrition) return;
      document.getElementById('saved-protein').oninput = e => { draft = applyEditorValue(draft, 'protein', e.target.value); updateSavedTotal(); };
      document.getElementById('saved-calories').oninput = e => { draft = applyEditorValue(draft, 'calories', e.target.value); updateSavedTotal(); };
      body.querySelectorAll('.ingredient-row input').forEach(input => input.addEventListener('input', () => { draft.ingredients = readIngredientRows(); draft = recalculateUnfixedTotals(draft); const updated = editorNutrition(draft); document.getElementById('saved-protein').value = roundInput(updated.proteinValue); document.getElementById('saved-calories').value = roundInput(updated.caloriesValue); updateSavedTotal(); }));
      ...
      document.getElementById('save-saved').onclick = () => { const name = document.getElementById('saved-name').value.trim(); if (!name) return showInlineError('saved-error', 'Enter a meal name.'); if (!hasNumberInput('saved-protein') || !hasNumberInput('saved-calories')) return showInlineError('saved-error', 'Enter both protein and calories.'); syncSavedDraft(); const meal = { id: existing?.id || draft.id || uid(), name, description: name, category: document.getElementById('saved-category').value, protein: num(draft.protein), calories: num(draft.calories), manualTotals: { ...draft.manualTotals }, ...(draft.per100Totals ? { per100Totals: { ...draft.per100Totals } } : {}), ingredients: activeIngredients(draft.ingredients), usageCount: existing?.usageCount || draft.usageCount || 0 }; const idx = state.savedMeals.findIndex(m => m.id === meal.id); if (idx >= 0) state.savedMeals[idx] = meal; else state.savedMeals.push(meal); saveState(); closeModal(); renderSaved(); };
      function syncSavedDraft() { if (body.querySelectorAll('.ingredient-row').length) draft.ingredients = readIngredientRows(); draft = applyEditorValue(draft, 'protein', document.getElementById('saved-protein').value); draft = applyEditorValue(draft, 'calories', document.getElementById('saved-calories').value); }
      function updateSavedTotal() { const el = document.getElementById('calculated-saved-total'), current = editorNutrition(draft); if (el && current.usesPer100) el.innerHTML = `For ${roundInput(current.amount)} g: <strong>${roundMacro(draft.protein)} g protein · ${Math.round(draft.calories)} kcal</strong>`; }
    }
  }

  function quickAdd(id) { const meal = state.savedMeals.find(m => m.id === id); if (!meal) return; meal.usageCount = (meal.usageCount || 0) + 1; getDay(selectedDate).entries.push({ id: uid(), category: MEAL_TYPES.includes(meal.category) ? meal.category : 'snacks', description: meal.description || meal.name, name: meal.name, protein: num(meal.protein), calories: num(meal.calories), manualTotals: normalizeManualTotals(meal.manualTotals, true), ...(meal.per100Totals ? { per100Totals: { ...meal.per100Totals } } : {}), ingredients: activeIngredients(meal.ingredients), source: 'saved', savedMealId: meal.id }); saveState(); renderToday(); toast(`${meal.name} added`); }
  ...
  function normalizePer100Totals(value, meal, amount = totalIngredientAmount(meal?.ingredients)) { if (!amount) return null; return { protein: Number.isFinite(Number(value?.protein)) ? num(value.protein) : num(meal?.protein) * 100 / amount, calories: Number.isFinite(Number(value?.calories)) ? num(value.calories) : num(meal?.calories) * 100 / amount }; }
  function editorNutrition(meal) { const amount = totalIngredientAmount(meal.ingredients), per100Totals = normalizePer100Totals(meal.per100Totals, meal, amount); return { amount, usesPer100: amount > 0, proteinValue: amount ? per100Totals.protein : meal.protein, caloriesValue: amount ? per100Totals.calories : meal.calories, per100Totals }; }
  ...
  function mealCopySnapshot(meal) {
    return { name: meal.name || meal.description || 'Meal', description: meal.description || '', category: MEAL_TYPES.includes(meal.category) ? meal.category : 'snacks', protein: num(meal.protein), calories: num(meal.calories), manualTotals: normalizeManualTotals(meal.manualTotals, true), ...(meal.per100Totals ? { per100Totals: { ...meal.per100Totals } } : {}), ingredients: activeIngredients(meal.ingredients), source: 'copy' };
  }
